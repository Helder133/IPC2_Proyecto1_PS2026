...
public class HistorialPagoService {
    ...
    public void insertar(HistorialPagoRequest historialPagoRequest) throws SQLException, UserDataInvalidException {
        HistorialPago historialPago = new HistorialPago(historialPagoRequest.getReservacionId(), historialPagoRequest.getMonto(), historialPagoRequest.getMetodo());
        if (historialPagoRequest.getFecha() != null) historialPago.setFecha(historialPagoRequest.getFecha());
        if (!historialPago.isValid()) throw new UserDataInvalidException("Los datos del pago son inválidos");
        Connection connection = DBConnection.getInstance().getConnection();
        connection.setAutoCommit(false);
        try {
    ...
    public List<HistorialPago> getHistorialPagoReservacion(int reservacionId) throws SQLException {
        HistorialPagoDAO historialPagoDAO = new HistorialPagoDAO();
        return historialPagoDAO.getHistorialPagoReservacion(reservacionId);
    }
